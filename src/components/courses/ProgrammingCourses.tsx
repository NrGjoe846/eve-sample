// src/components/courses/ProgrammingCourses.tsx
import React from 'react';
import { Code2, BookOpen, Trophy, Star, Clock, Scroll, Zap, Skull, Flame, ChevronLeft } from 'lucide-react';
...
const courses: Course[] = [
  ...
  },
  {
    ...
    description: 'Race through Java’s digital highways, mastering circuits of code in a high-tech arena',
    level: 'Beginner',
    duration: '12 weeks',
    xp: 1500,
    progress: 25,
  ...
  },
  {
    ...
    description: 'Survive the horrors of C, mastering its cursed depths in a ghostly realm',
    level: 'Beginner',
    duration: '10 weeks',
    xp: 2000,
    progress: 10,
  ...
  return (
    <div className="min-h-screen bg-[url('https://i.pinimg.com/originals/e2/86/14/e28614bd3f7f44fc686a28d14063e5b8.jpg')] bg-cover bg-center text-gray-200 p-8 font-sans">
      ...
      <div className="mb-8 text-center">
        <h2 className="text-3xl font-bold mb-4 text-[#f4e4bc]">Realms of Code Mastery</h2>
        <p className="text-[#80deea]">Embark on epic quests to conquer coding challenges</p>
      </div>
        ...
        {courses.map((course) => {
          ...
          return (
            <div key={course.id} className="group relative h-[600px]">
              {/* Glow Effect */}
              <div className={`absolute inset-0 ${glowTheme} rounded-xl blur-xl transition-all duration-300 opacity-0 group-hover:opacity-100 z-10`} />
              {/* Course Card */}
              <div className={`relative rounded-xl border overflow-hidden transition-all duration-300 h-full flex flex-col ${cardTheme} z-20`}>
                <div className="p-6 flex-1">
                  <div className="flex items-center justify-between mb-4">
                    <span className={`px-3 py-1 rounded-full text-sm ${
                      isPythonFundamentals ? 'bg-[#6b4e31]/20 text-[#f4e4bc]' :
                  ...
                  <div className="mb-4">
                    <div className={`flex justify-between text-sm mb-1 ${secondaryTextTheme}`}>
                      <span>{isPythonFundamentals ? 'Journey Progress' : isJavaCyber ? 'Race Progress' : 'Survival Progress'}</span>
                  ...
                  <div className="space-y-3 mb-6">
                    {course.chapters.map((chapter) => (
                      <div
                        key={chapter.id}
                        className={`flex items-center justify-between p-3 rounded-lg ${chapter.unlocked ? chapterBgUnlocked : chapterBgLocked}`}
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className={`text-xs ${secondaryTextTheme}`}>
                              {chapter.completed}/{chapter.lessons} {isPythonFundamentals ? 'trials' : isJavaCyber ? 'laps' : 'echoes'}
                            </p>
                          </div>
                        </div>
                        {chapter.unlocked ? (
                          <Trophy className={`w-4 h-4 ${
                            chapter.completed === chapter.lessons 
                              ? isPythonFundamentals ? 'text-[#a67c00]' : isJavaCyber ? 'text-[#00d4ff]' : 'text-[#8B0000]'
                              : isPythonFundamentals ? 'text-[#6b4e31]' : isJavaCyber ? 'text-[#80deea]' : 'text-[#468284]'
                          }`} />
                        ) : (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={textTheme}>
                        {course.xp} {isPythonFundamentals ? 'Valor' : isJavaCyber ? 'Speed' : 'Dread'}
                      </span>
                    </div>
                    {course.path ? (
                      <Link
                      ...
                      >
                        {isPythonFundamentals ? 'Embark on Quest' : isJavaCyber ? 'Start Race' : 'Enter Mausoleum'}
                      </Link>
                    ) : (
                      <button className={`px-4 py-2 rounded-lg ${disabledButtonTheme} font-bold`} disabled>
                        Coming Soon
                      </button>
                    )}
                  </div>
